...
export const Route = createFileRoute("/patient/journal")({
...
function JournalPage() {
  ...
  const [selected, setSelected] = useState<string | null>(open ?? JOURNAL_ENTRIES[0]?.id ?? null);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Wellness journal</h1>
        <p className="mt-2 max-w-xl text-sm text-muted-foreground">
          Track your mood day to day and jot down what's on your mind — your care team can see
        ...
        <section className="relative overflow-hidden rounded-3xl bg-foreground p-6 text-background sm:p-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-xl font-black tracking-tight">How are you feeling?</h2>
              <div className="mt-4 flex gap-2">
                {([1, 2, 3, 4, 5] as Mood[]).map((m) => (
                  <button
                    key={m}
                    onClick={() => handleCheckIn(m)}
                    className={`grid h-12 w-12 place-items-center rounded-2xl text-2xl transition hover:-translate-y-0.5 ${
            ...
            <div className="grid grid-cols-3 gap-4 rounded-2xl border border-background/15 bg-background/10 p-5 text-center backdrop-blur-md">
              <div>
                <p className="text-2xl font-black">{WELLNESS_STATS.streakDays}</p>
                <p className="mt-1 text-[10px] uppercase tracking-widest text-background/60">
                  Day streak
                </p>
              </div>
              <div>
                <p className="text-2xl font-black">{WELLNESS_STATS.avgMood}</p>
                <p className="mt-1 text-[10px] uppercase tracking-widest text-background/60">
                  Avg mood
                </p>
              </div>
          ...
          <form onSubmit={handleSaveEntry} className="mt-4 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <input
                ...
                className="w-32 rounded-full border border-border bg-background px-3 py-1.5 text-xs outline-none focus:border-foreground"
              />
            </div>
            <button
              type="submit"
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground line-clamp-2">{e.content}</p>
